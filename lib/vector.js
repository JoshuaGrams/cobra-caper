...
var V2 = {};
V2.eq = function(u, v) { return u[0] === v[0] && u[1] === v[1]; }
...
function Transform2D(position, rotation, scale) {
	...
	this.scale = scale || 1;
	// cached matrix and parameters
	this.p = [];  this.r = false;  this.s = false;
	this.m = false;
}

Transform2D.prototype.set = function(position, rotation, scale) {
	if(typeof position !== "undefined") this.position = position;
	if(typeof rotation !== "undefined") this.rotation = rotation || 0;
	if(typeof scale !== "undefined") this.scale = scale || 1;
}
...
Transform2D.prototype.matrix = function() {
	if(!(V2.eq(this.position, this.p) && this.rotation === this.r
		&& (V2.eq(this.scale, this.s) || this.scale === this.s))) {
			this.m = M3x2.transform(this.position, this.rotation, this.scale);
			this.p = this.position.slice();
			this.r = this.rotation;
			if(this.scale[0]) this.s = this.scale.slice();
			else this.s = this.scale;
		}
	return this.m;
}
...
// If there are real-valued solutions, return them in a 2-element array.
function solveQuadratic(a, b, c) {
	// Divide by a, then halve b to get: x^2 - 2bx + c.
	// Then solutions are: b +/- sqrt(b^2 - c).
	b = b/(-2*a);  c = c/a;
	// If the discriminant is negative there are no real solutions.
	var d = b*b - c;  if(d < 0) return;
	// For numerical stability, use the one with larger magnitude.
	var x0 = b + Math.sign(b) * Math.sqrt(d);
