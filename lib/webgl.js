...
(function() {
	...
	WebGL.errorChecking = false;

	WebGL.check = function(gl) {
		if(!WebGL.errorChecking) return;
		var err = gl.getError();
		if(!err) return;
		switch(err) {
	...
	WebGL.TypeInfo = {
		...
		,'indices': { size: 2, type: WebGL.UNSIGNED_SHORT,
	...
	WebGL.Shader = function(gl, prog, mode, variables, buffers) {
		...
		if(buffers) for(var i=0; i<buffers.length; ++i) {
			...
			var buf = new WebGL.VertexBuffer(gl, prog, attributes);
			this.buffers.push(buf);
			if(buf.indexType) this.indexType = buf.indexType;
		}
	...
	WebGL.ShaderFrom = function(gl, vertex, fragment, mode, variables, buffers) {
		var prog = WebGL.buildProgram(gl, vertex, fragment);
		WebGL.Shader.bind(this)(gl, prog, mode, variables, buffers);
	}
	WebGL.ShaderFrom.prototype = WebGL.Shader.prototype;
	...
	WebGL.Shader.prototype.draw = function(values, vertexData, firstVertex, numVertices) {
		this.set(values, vertexData);
		this.process(firstVertex, numVertices);
	}

	WebGL.Shader.prototype.set = function(values, vertexData) {
		...
		if(values) for(var name in values) {
	...
	WebGL.Shader.prototype.process = function(firstVertex, numVertices) {
		if(this.indexType) {
			this.gl.drawElements(this.mode, numVertices,
			   	this.indexType.type, firstVertex*this.indexType.size);
			WebGL.check(this.gl);
		} else {
			this.gl.drawArrays(this.mode, firstVertex, numVertices);
		  	WebGL.check(this.gl);
		}
	}
	...
	WebGL.VertexBuffer.prototype.set = function(gl, data) {
		...
		if(data) {
			if(!(data instanceof this.array)) data = new this.array(data);
	...
	WebGL.Variable = function(gl, prog, name, type) {
		...
		if(matrix !== '') {
			this.set = function(gl, value) { gl[fn](this.loc, false, M3x2.toFloat32(value, true)); }
	...
	WebGL.Texture = function(gl, image, format, nearest) {
		...
		if(nearest) {
			...
			gl.texParameteri(bp, gl.TEXTURE_MIN_FILTER, gl.LINEAR);  WebGL.check(gl);
		}
		return texture;
	}
